...
- **牛顿下山条件** (仅牛顿法): 启用后保证 |f(x_{n+1})| < |f(x_n)|
...
用两点连线的割线代替切线，避免计算导数。超线性收敛（收敛阶约为黄金比例 φ ≈ 1.618）。
...
**注**: 单点弦截法需要预先给定斜率且对初值敏感、收敛性不稳定，故此处仅展示双点弦截法。
...
- **Primary (主题绿色)**: `#006C39` - 函数曲线（50% 透明度）
- **Secondary (主题棕色)**: `#A13F0B` - 坐标轴（30% 透明度）
- **Accent**: 蓝紫渐变 - 标题和强调元素
- **中性色**: Slate 系列 - 文字和边框
